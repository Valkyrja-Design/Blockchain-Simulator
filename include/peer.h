...
enum type {HONEST, SELFISH, STUBBORN};
class peer{
    private:
        int id;
        speed label1;
    ...
    public:
        ...
        type miner;
        int lead;                   //if selfish what lead he has over honest miners?
        ...
        std::shared_ptr<block> topBlockToAttack;
        std::deque<std::shared_ptr<block>> privatePool;                  //Private pool of blocks only for selfish miner

        peer(int coins)
            : coins(coins) {}
        peer(speed label1, cpu_power label2, int coins) 
            : id(++peer::peer_no), label1(label1), label2(label2), coins(coins), miner(type::HONEST) {}
        ...
        void setMiner(type minerType);
        void addToPrivatePool(std::shared_ptr<block> blk);
        void print_pvt_pool(std::fstream& file) const;
